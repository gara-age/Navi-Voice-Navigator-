...
$exe = Join-Path $PSScriptRoot 'dist\connected\voice_navigator.exe'
...
if (Test-Path $exe) {
  ...
  Write-Host "Source files are newer than the connected build. Launching current Flutter code..." -ForegroundColor Yellow
}

if (-not (Test-Path $exe)) {
  Write-Host "No built connected executable found. Launching current Flutter code..." -ForegroundColor Yellow
}

powershell -ExecutionPolicy Bypass -File .\scripts\run_flutter_connected.ps1
